...
def send_email_reminder(approver_email, transaction_id, approver_id, process_name, process_description):
    ...
    approve_url = url_for('transaction.update_approval_status', transaction_id=transaction_id, status='approved', approver_id=approver_id, _external=True)
    reject_url = url_for('transaction.update_approval_status', transaction_id=transaction_id, status='rejected', approver_id=approver_id, _external=True)
    changes_url = url_for('transaction.request_changes', transaction_id=transaction_id, approver_id=approver_id, _external=True)
...
@transaction_bp.route('/transactions/<transaction_id>/request_changes', methods=['GET', 'POST'])
def request_changes(transaction_id):
    ...
    if request.method == 'POST':
        ...
        if not new_approver_id:
            return jsonify({"error": "New Approver ID not provided"}), 400
        ...
        transaction['approvals'].append(new_approval_entry)
        ...
        return jsonify({"message": "New approver added and status updated", "transaction": str(transaction['_id'])}), 200

    return '''
        <form method="post">
            New Approver ID: <input type="text" name="new_approver_id"><br>
...
#             process = current_app.mongo.db.processes.find_one({"_id": ObjectId(transaction['processId'])})
#             process_name = process['name'] if process else 'N/A'
#             process_description = process['description'] if process else 'N/A'
...
#     # Update the overall status based on individual statuses
#    # update_overall_status(transaction_id)
...
# def update_overall_status(transaction_id):
#     transaction = current_app.mongo.db.transactions.find_one({"_id": ObjectId(transaction_id)})

#     if not transaction:
#         return

#     statuses = [approval['status'] for approval in transaction['approvals']]
#     if 'rejected' in statuses:
#         overall_status = 'rejected'
#     elif all(status in ['approved', 'changes_requested'] for status in statuses):
...
#     current_app.mongo.db.transactions.update_one(
#         {"_id": ObjectId(transaction_id)},
#         {"$set": {"status": overall_status, "updated_at": datetime.utcnow()}}
#     )
