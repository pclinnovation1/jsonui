...
def calculate_performance(goals, max_bonus, annual_overall_salary):
    ...
    for goal in goals:
        ...
        results.append({
            ...
            'estimated_result': goal.get('estimated_result', goal.get('estimated_result_percent')),
            'estimated_target_result_percent': f"{target_result_percent * 100:.2f}%",
            'estimated_target_result_weighted_percent': f"{weighted_target_result_percent * 100:.2f}%",
            'amount': f"{amount:.2f}"
        })
...
def main():
    ...
    for goal_data in goal_plans:
        plan_name = goal_data.get("name", "Unnamed Plan")
