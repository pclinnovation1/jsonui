...
# Function to create online meeting
def create_online_meeting(access_token, user_id):
    ...
    meeting_data = {
        "startDateTime": (now_utc + timedelta(minutes=5)).isoformat(),
        "endDateTime": (now_utc + timedelta(hours=1)).isoformat(),
...
if __name__ == "__main__":
    ...
    for data in data_from_db:
        teams_link = create_online_meeting(access_token, user_id)
        now_utc = datetime.now(timezone.utc)
        event_data = {
            'subject': data.get('subject', 'No Subject'),
            'start': {
                'dateTime': data.get('start_time', now_utc.isoformat()),  # Default to current time if not provided
                'timeZone': 'UTC'
            },
            'end': {
                'dateTime': data.get('end_time', (now_utc + timedelta(hours=1)).isoformat()),  # Default to one hour from now if not provided
                'timeZone': 'UTC'
            },
            'location': {
