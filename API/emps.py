...
def update_assignment_with_names(assignment, mappings):
    ...
    """
    assignmentidname={}
    assignmentidname["job_name"] = mappings["jobs"].get(assignment.get("JobId"), "NA")
    assignmentidname["position_name"] = mappings["positions"].get(assignment.get("PositionId"), "NA")
    assignmentidname["grade_name"] = mappings["grades"].get(assignment.get("GradeId"), "NA")
    assignmentidname["location_name"] = mappings["locations"].get(assignment.get("LocationId"), "NA")
    return assignmentidname
...
def map_assignment_data(data):
    ...
    latest_updated_assignment= None

    global mappingsidname
    ...
    for work_relationship in data.get("workRelationships", []):
        for assignment in work_relationship.get("assignments", []):
            start_date_str = assignment.get("EffectiveStartDate")
             # Update assignment data with names
            updated_assignment = update_assignment_with_names(assignment, mappingsidname)
            ...
            if start_date_str:
                ...
                if start_date > latest_start_date:
                    ...
                    latest_updated_assignment=updated_assignment
    ...
    if latest_assignment and latest_work_relationship:
        ...
        return {**work_relationship_data, **assignment_data,**latest_updated_assignment}
...
def fetch__jobs_id_to_jobs_name_mapping():
    api_config = CONFIG["api"]
    offsets = list(range(
        CONFIG["offsets"]["range_start"],
        CONFIG["offsets"]["range_end"],
    ...
    def fetch_workers_chunk(offset):
        url = f"{api_config['url']}/hcmRestApi/resources/11.13.18.05/jobsLov?fields=JobId,JobName&limit={api_config['limit']}&offset={offset}&onlyData=true"
        response = requests.get(url, auth=(api_config['username'], api_config['password']))
        response.raise_for_status()
        return response.json().get('items', [])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        for future in concurrent.futures.as_completed([executor.submit(fetch_workers_chunk, offset) for offset in offsets]):
            ...
            for worker in workers:
                person_mapping[worker["JobId"]] = worker["JobName"]

    return person_mapping

# Function to fetch Person ID to Person Number mapping
def fetch__positions_id_to_positions_name_mapping():
    api_config = CONFIG["api"]
    offsets = list(range(
        CONFIG["offsets"]["range_start"],
        CONFIG["offsets"]["range_end"],
    ...
    # Function to fetch a chunk of worker data
    def fetch_workers_chunk(offset):
        url = f"{api_config['url']}/hcmRestApi/resources/11.13.18.05/positionsLov?fields=PositionId,PositionName&limit={api_config['limit']}&offset={offset}&onlyData=true"
        response = requests.get(url, auth=(api_config['username'], api_config['password']))
        response.raise_for_status()
        return response.json().get('items', [])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        for future in concurrent.futures.as_completed([executor.submit(fetch_workers_chunk, offset) for offset in offsets]):
            workers = future.result()
            for worker in workers:
                person_mapping[worker["PositionId"]] = worker["PositionName"]

    return person_mapping

# Function to fetch Person ID to Person Number mapping
def fetch__grades_id_to_grades_name_mapping():
    api_config = CONFIG["api"]
    offsets = list(range(
        CONFIG["offsets"]["range_start"],
        CONFIG["offsets"]["range_end"],
    ...
    # Function to fetch a chunk of worker data
    def fetch_workers_chunk(offset):
        url = f"{api_config['url']}/hcmRestApi/resources/11.13.18.05/gradesLov?fields=GradeId,Name&limit={api_config['limit']}&offset={offset}&onlyData=true"
        response = requests.get(url, auth=(api_config['username'], api_config['password']))
        response.raise_for_status()
        return response.json().get('items', [])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        for future in concurrent.futures.as_completed([executor.submit(fetch_workers_chunk, offset) for offset in offsets]):
            workers = future.result()
            for worker in workers:
                person_mapping[worker["GradeId"]] = worker["Name"]

    return person_mapping

# Function to fetch Person ID to Person Number mapping
def fetch__locations_id_to_locations_name_mapping():
    api_config = CONFIG["api"]
    offsets = list(range(
        CONFIG["offsets"]["range_start"],
        CONFIG["offsets"]["range_end"],
    ...
    # Function to fetch a chunk of worker data
    def fetch_workers_chunk(offset):
        url = f"{api_config['url']}/hcmRestApi/resources/11.13.18.05/locationsLov?fields=LocationId,LocationName&limit={api_config['limit']}&offset={offset}&onlyData=true"
        response = requests.get(url, auth=(api_config['username'], api_config['password']))
        response.raise_for_status()
        return response.json().get('items', [])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        for future in concurrent.futures.as_completed([executor.submit(fetch_workers_chunk, offset) for offset in offsets]):
            workers = future.result()
            for worker in workers:
                person_mapping[worker["LocationId"]] = worker["LocationName"]
...
def generate_report3():
    ...
    try:
        ...
        print("workers_data done")
        emps_data = fetch_emps_data()
        print("emps_data done")
        # Create mappings
        mappingsidname = {
            "jobs": fetch__jobs_id_to_jobs_name_mapping(),
            "positions": fetch__positions_id_to_positions_name_mapping(),
            "grades": fetch__grades_id_to_grades_name_mapping(),
            "locations": fetch__locations_id_to_locations_name_mapping()
        }
        
        print("mappingsidname done")
