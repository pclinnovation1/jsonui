...
derived_employee_collection = db[config.COLLECTIONS["derived_employee_collection"]]
...
my_goals_plan_collection = db[config.COLLECTIONS["my_goals_plan_collection"]]
goal_offering_collection = db[config.COLLECTIONS["goal_offering_collection"]]
mass_assign = db[config.COLLECTIONS["mass_assign"]]
goals_created_by_employees = db[config.COLLECTIONS["goals_created_by_employees"]]
email_queue = db[config.COLLECTIONS["email_queue"]]
...
def send_email_on_employee_goal_created(goal_data):
    email_data = {
        "person_name": goal_data["manager_name"],
        "from_email": "PGM_department",
        "template_name": "employee_goal_created",
        "data": {
            "manager_name": goal_data["manager_name"],
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
            "goal_plan_assigned": goal_data["goal_plan_assigned"],
            "review_period":goal_data['review_period'],
            "performance_document_type":goal_data['performance_document_type']
        }
    }
...
def send_email_on_goal_assignment_employee(goal_data):
    email_data = {
        "person_name": goal_data["manager_name"],
        "from_email": "PGM_department",
        "template_name": "goal_assign_employee",
        "data": {
            "manager_name": goal_data["manager_name"],
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
...
def send_email_on_goal_date_or_goal_type_updated(goal_data):
    # Format relevant changes for HTML
    relevant_changes_html = ''
    for change in goal_data['relevant_changes']:
        relevant_changes_html += f"<li style='font-weight: bold; color: #FF0000; margin-bottom: 5px;'>{change}</li>"

    # Pass the formatted HTML string to the email template
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_date_or_goal_type_updated",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
            ...
            "review_period": goal_data["review_period"],
            "performance_document_type": goal_data["performance_document_type"],
            "relevant_changes": relevant_changes_html  # Pass the formatted changes
        }
    }
    queue_email(email_data)

def send_email_on_goal_new_measurement_add(goal_data):
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_new_measurement_addition",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
...
def send_email_on_goal_measurement_updated(goal_data):
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_measurement_updation",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
...
def send_email_on_goal_new_task_add(goal_data):
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_new_task_addition",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
...
def send_email_on_goal_task_updated(goal_data):
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_task_updation",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
...
def send_email_on_goal_task_progress_update(goal_data):
    email_data = {
        "person_name": goal_data["manager_name"],
        "from_email": "PGM_department",
        "template_name": "goal_task_progress_updated",
        "data": {
            "manager_name": goal_data["manager_name"],
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
...
def send_email_on_feedback_of_goal(goal_data):
    email_data = {
        ...
        "data": {
            ...
            "review_period":goal_data['review_period'],
            "performance_document_type":goal_data['performance_document_type'],
            "feedback":goal_data['feedback']
        }
    }
    queue_email(email_data)

def send_email_on_goal_comment_add(goal_data):
    email_data = {
        "person_name": goal_data["person_name"],
        "from_email": "PGM_department",
        "template_name": "goal_comment",
        "data": {
            "person_name": goal_data["person_name"],
            "goal_name": goal_data["goal_name"],
            "goal_type": goal_data["goal_type"],
...
def create_goal():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys

    # Validate input JSON against the schema
    try:
        validate(instance=data, schema=config.goal_schema)
...
@goal_bp.route('/create_goal_employee', methods=['POST'])
def create_goal_employee():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys
    ...
    if 'updated_by' not in data:
        return jsonify({"error": "missing updated_by"}), 400
    
    worker_exists = employee_details_collection.find_one({'person_name': data.get("updated_by")})
    ...
    data["updated_at"] = current_time
    m=employee_details_collection.find_one({'person_name': worker_exists.get("manager_name")})
    prep_data={
            "manager_name": m["person_name"],
            "person_name": worker_exists["person_name"],
            "goal_name": worker_exists["goal_name"],
            "goal_type": worker_exists["goal_type"],
            "goal_plan_assigned": worker_exists["goal_plan_assigned"],
            "review_period":worker_exists['review_period'],
            "performance_document_type":worker_exists['performance_document_type']
    }
    # Insert the new goal into the goals collection
    goals_collection.insert_one(data).inserted_id
    # send email to manager regarding above
    send_email_on_employee_goal_created(prep_data)
    data["created_by"]=data.get('updated_by')
    goals_created_by_employees.insert_one(data)
    return jsonify({"message":"goal created"}), 201
...
def add_goal():
    ...
    if data.get('measurements', {}):
       ...
       for measurement in data.get('measurements'):
           # Check if any required fields are missing
            missing_measurement_fields = [field for field in config.allowed_measurement_fields if field not in measurement]
            ...
            extra_fields = [field for field in measurement if field not in config.allowed_measurement_fields]
            if extra_fields:
    ...
    if data.get('tasks', {}):
       ...
       for task in data.get('tasks'):
           # Check if any required fields are missing
            missing_task_fields = [field for field in config.allowed_task_fields if field not in task]
            if missing_task_fields:
                return jsonify({"error": "Missing fields in task", "missing": missing_task_fields}), 400

            # Check for any extra fields
            extra_fields = [field for field in task if field not in config.allowed_task_fields]
            if extra_fields:
...
@goal_bp.route('/assign_employee', methods=['POST'])
def add_goal_employee():
    data = config.lowercase_keys(request.json)
    required_fields = ["person_name", "goal_name"]  # Define required fields
    missing_fields = [field for field in required_fields if field not in data]
    if missing_fields:
        return jsonify({"error": "Missing required fields", "missing": missing_fields}), 400
    ...
    goal_data = {
        ...
        "comments": data.get("comments", []),
        "feedback": data.get("feedback", []),
        "updated_by": data.get("person_name"),
        "updated_at": current_time
    }
    ...
    # Insert the goal into the my_goals collection
    goal_id = my_goals_collection.insert_one(goal_data).inserted_id
    goal_data['manager_name']= m["person_name"]
    # send email to manager regardiong this
    send_email_on_goal_assignment_employee(goal_data)
    new_goal = my_goals_collection.find_one({'_id': goal_id})
    new_goal['_id'] = str(new_goal['_id'])
    return jsonify(new_goal), 201
...
def goal_offering():
    ...
    if existing_goal1:
        ...
        # Convert dates to datetime objects for comparison (assuming they are strings in ISO format)
        existing_target_completion_date1 = datetime.strptime(existing_target_completion_date1, '%Y-%m-%d')
    ...
    send_email_on_goal_offering(goal_data)
    new_goal['_id'] = str(new_goal['_id'])
...
def process_goal():
    ...
    if status == "approved":
        ...
        send_email_on_goal_assignment(my_goals_collection)
        goal_offering_collection.update_one(
            {
                "goal_offering_data": goal
                # "person_name": person_name,
                # "goal_name": goal_name,
...
@goal_bp.route('/overwrite_measurementsg', methods=['POST'])
def overwrite_measurementsg_in_goal():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys
    ...
    # Ensure measurement data is provided and is a list
    if not measurement_data or not isinstance(measurement_data, list):
        return jsonify({"error": "No valid measurement data provided"}), 400

    # Validate the format of each measurement
    for measurement in measurement_data:
        # Check for extra fields that are not allowed
        extra_fields = set(measurement.keys()) - config.allowed_measurement_fields
        if extra_fields:
            return jsonify({"error": f"Unexpected fields in measurement: {', '.join(extra_fields)}"}), 400

        # Check for missing fields
        missing_fields = config.allowed_measurement_fields - set(measurement.keys())
        if missing_fields:
            return jsonify({"error": f"Missing required fields in measurement: {', '.join(missing_fields)}"}), 400

    # Overwrite the 'measurements' array in the goal document
    update_data = {
        "$set": {
            "measurements": measurement_data,  # Overwrite the measurements array with the new data
            "updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            "updated_by": data.get('updated_by', 'Admin')  # Track who performed the update
        }
    }

    # Perform the update in the 'goals_collection'
    result = goals_collection.update_one({"basic_info.goal_name": goal_name1}, update_data)

    # Check if the update was successful
    if result.modified_count == 0:
        return jsonify({"error": "No matching goal found or no update performed"}), 404

    return jsonify({
        "message": "Measurements overwritten successfully"
    }), 200
...
@goal_bp.route('/overwrite_tasksg', methods=['POST'])
def overwrite_tasksg_in_goal():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys
    ...
    # Ensure tasks data is provided and is a list
    if not tasks_data or not isinstance(tasks_data, list):
        return jsonify({"error": "No valid tasks data provided"}), 400

    # Validate the format of each task
    for task in tasks_data:
        # Check for extra fields that are not allowed
        extra_fields = set(task.keys()) - config.allowed_task_fields
        if extra_fields:
            return jsonify({"error": f"Unexpected fields in task: {', '.join(extra_fields)}"}), 400

        # Check for missing fields
        missing_fields = config.allowed_task_fields - set(task.keys())
        if missing_fields:
            return jsonify({"error": f"Missing required fields in task: {', '.join(missing_fields)}"}), 400

    # Overwrite the 'tasks' array in the goal document
    update_data = {
        "$set": {
            "tasks": tasks_data,  # Overwrite the tasks array with the new data
            "updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            "updated_by": data.get('updated_by', 'Admin')  # Track who performed the update
        }
    }
    ...
    return jsonify({
        "message": "Tasks overwritten successfully"
    }), 200
...
def filter_and_update_my_goals():
    ...
    if update_data:
        ...
        # Loop through the matched goals and send emails
        for goal in matched_goals:
            person_name = goal.get("person_name")
            
            # Determine if the relevant fields were updated
            relevant_changes = []
            if 'start_date' in update_fields:
                relevant_changes.append(f"Start Date updated to {update_fields['start_date']}")
            if 'target_completion_date' in update_fields:
                relevant_changes.append(f"Target Completion Date updated to {update_fields['target_completion_date']}")
            if 'goal_type' in update_fields:
                relevant_changes.append(f"Goal Type updated to {update_fields['goal_type']}")
            ...
            if relevant_changes:
                prep_data={
                    ...
                    "performance_document_type":goal['performance_document_type'],
                    "relevant_changes":relevant_changes   
                }   
                send_email_on_goal_date_or_goal_type_updated(prep_data)
        return jsonify({
...
def update_goal_progress():
    ...
    e1=employee_details_collection.find_one({'person_name': data.get("person_name")})
    m=employee_details_collection.find_one({'person_name': e1.get("manager_name")})
    prep_data={
            "manager_name": m["person_name"],
            "person_name": worker_exists["person_name"],
            "goal_name": worker_exists["goal_name"],
    ...
    send_email_on_goal_progress_updated(prep_data)
    return jsonify({
...
def add_measurement_to_goal():
    ...
    for goal in matched_goals:
            ...
            send_email_on_goal_new_measurement_add(prep_data)
    return jsonify({
        "message": "Measurement added successfully",
        "matched_count": result.matched_count,
        "modified_count": result.modified_count
...
@goal_bp.route('/overwrite_measurements', methods=['POST'])
def overwrite_measurements_in_goal():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys

    # Separate filter criteria from measurement data
    filter_criteria = data.get('filter', {})
    measurements = data.get('measurements', [])  # Expecting a list of measurements
    ...
    # Ensure measurements data is provided and is a list
    if not measurements or not isinstance(measurements, list):
        return jsonify({"error": "No valid measurements data provided"}), 400

    # Validate the format of each measurement
    for measurement in measurements:
        # Check that all required fields are present and no extra fields are included
        extra_fields = set(measurement.keys()) - config.allowed_measurement_fields
        if extra_fields:
            return jsonify({"error": f"Unexpected fields in measurement: {', '.join(extra_fields)}"}), 400
    ...
    # Prepare the filter query for MongoDB to find the goal
    query = {}
    for key, value in filter_criteria.items():
        formatted_key = key.lower().replace(' ', '_')
        query[formatted_key] = value
    ...
    # Prepare the update operation to overwrite the 'measurements' array
    update_data = {
        "$set": {
            "measurements": measurements,  # Overwrite the measurements array with the new measurements
            "updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            "updated_by": data.get('updated_by', 'Admin')
        }
    }
    ...
    for goal in matched_goals:
        ...
        send_email_on_goal_measurement_updated(prep_data)
...
def add_task_to_goal():
    ...
    for goal in matched_goals:
            ...
            send_email_on_goal_new_task_add(prep_data)
    return jsonify({
...
@goal_bp.route('/overwrite_tasks', methods=['POST'])
def overwrite_tasks_in_goal():
    data = config.lowercase_keys(request.json)  # Ensure data is consistent with lowercase keys

    # Separate filter criteria from task data
    filter_criteria = data.get('filter', {})
    tasks = data.get('tasks', [])  # Expecting a list of tasks
    ...
    # Prepare the update operation to overwrite the 'tasks' array
    update_data = {
        "$set": {
            "tasks": tasks,  # Overwrite the tasks array with the new tasks
            "updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            "updated_by": data.get('updated_by', 'Admin')  # Optionally track who updated the tasks
        }
    }
    ...
    for goal in matched_goals:
        ...
        send_email_on_goal_task_updated(prep_data)
    # Return the result of the operation
    return jsonify({
        "message": "Tasks overwritten successfully",
        "matched_count": result.matched_count,
...
def update_task_status():
    ...
    try:
        ...
        prep_data={
                        "manager_name": m["person_name"],
                        "person_name": goal_document['person_name'],
                        "goal_name": goal_document["goal_name"],
                        "goal_type": goal_document["goal_type"],
                        "goal_plan_assigned": goal_document["goal_plan_assigned"],
                        "review_period":goal_document['review_period'],
                        "performance_document_type":goal_document['performance_document_type'],
                        "task_name":data.get('task_name'),
                        "status": data.get('status')  
        }
        send_email_on_goal_task_progress_update(prep_data)
        return jsonify({
...
def update_task_completion():
    ...
    try:
        ...
        send_email_on_goal_task_completion_update(prep_data)
        return jsonify({
...
def add_feedback_to_goal():
    ...
    fdata= {
        ...
        "given_by": feedback_data.get('given_by','Admin')
    }
    ...
    update_query = {
        ...
        "$set": {
            "updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            "updated_by": feedback_data.get('given_by','Admin')
        }
    }
    ...
    try:
        ...
        send_email_on_feedback_of_goal(prep_data)
        return jsonify({
...
def add_comment_to_goal():
    ...
    update_data = {
        ...
        "$set": {"updated_at": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                 "updated_by": data.get('given_by','Admin')}
    }
    ...
    for goal in matched_goals:
            ...
            send_email_on_goal_comment_add(prep_data)
    return jsonify({
