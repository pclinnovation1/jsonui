...
performance_document_types_collection = db["PGM_performance_document_types"]
...
def create_goal_plan():
    ...
    if goal_plan_exists:
        return jsonify({"error": "A goal plan with this name and review period already exists"}), 400
...
def add_workers_to_goal_plan():
    ...
    return jsonify({
        ...
        "updated_goal_plan": updated_goal_plan
    }), 200
...
def overwrite_workers_in_goal_plan():
    ...
    # # Return the updated goal plan
    # updated_goal_plan = goal_plan_collection.find_one({"details.goal_plan_name": })
    # updated_goal_plan['_id'] = str(updated_goal_plan['_id'])  # Convert ObjectId to string for JSON response
...
def add_eligibility_profiles_to_goal_plan():
    ...
    if 'eligibility_profiles' in data:
        ...
        for profile in new_eligibility_profiles:
            profile_data = eligibility_profile_collection.find_one({"eligibility_profile_definition.name": profile["name"]})
            if profile_data:
