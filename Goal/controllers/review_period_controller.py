...
@review_period_bp.route('/<review_period_name>', methods=['GET'])
def get_review_period(review_period_name):
    review_period = review_period_collection.find_one({'review_period_name': review_period_name})
    if review_period:
        review_period['_id'] = str(review_period['_id'])
...
@review_period_bp.route('/<review_period_name>', methods=['PUT'])
def update_review_period(review_period_name):
    ...
    result = review_period_collection.update_one({'review_period_name': review_period_name}, {'$set': data})
    if result.matched_count:
        updated_review_period = review_period_collection.find_one({'review_period_name': review_period_name})
        updated_review_period['_id'] = str(updated_review_period['_id'])
        return jsonify(updated_review_period), 200
    else:
        return jsonify({'error': 'Review period not found'}), 404

@review_period_bp.route('/<review_period_name>', methods=['DELETE'])
def delete_review_period(review_period_name):
    result = review_period_collection.delete_one({'review_period_name': review_period_name})
